    ...
    >
      ...
      { (showHead || showText || showLink) &&
        <Box {...slideOverrides.content}>
          { showHead &&
            <Text {...slideOverrides.head} />
          }
          { showText &&
            <Text {...slideOverrides.text} />
          }
          { showLink &&
            <Link {...slideOverrides.link} />
          }
        </Box>
      }
    </Box>
...
const CarouselComponent = ({
  ...
  const setSlide = useCallback((active, position, animate, lock) => {
    ...
    const sliderWidth = el instanceof Element
      ? el.offsetWidth
  ...
  return (
    ...
    >
      <Box
        ...
        transition={`transform ${animate ? duration : 0}ms ${functionProp}`}
      >
        { list.map((numb, index) => (
          <Slide
            key={`slide-${index}`}
