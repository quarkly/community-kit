...
const Gallery = ({
  ...
  const imagesMaxWidth = useMemo(() => 
    strToNumb(imagesMaxWidthProp),
  [imagesMaxWidthProp]);
  
  const imagesMinWidth = useMemo(() => 
    strToNumb(imagesMinWidthProp),
  [imagesMinWidthProp]);
  ...
  const getItemWidth = useCallback(galleryWidth => 
    (galleryWidth - ((columnsCountProp - 1) * borderWidthProp)) / columnsCountProp
  ,[
    columnsCountProp, 
    borderWidthProp, 
    columnsCountProp,
  ...
  const items = Array(
    itemsLoadingNumb > 0 ? parseInt(itemsLoadingNumb) : 0
  ).fill().map((item, index) =>
    (
      <GalleryItem
  ...
  return (
    <Box {...rest}>
      ...
      <Button
        onClick={loadOnClick}
        {...override(`Button More`, `Button More ${(isButtonVisible) ? ':visible' : ':hidden'}`)}
      >
